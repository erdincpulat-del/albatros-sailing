...
const MARINA = {
  ...
  description: "Klasik marina sahnesi ve yelkenli yat iskele yanaşma eğitimi.",
...
const BOAT: BoatConfig = {
  label: "Sailing Yacht",
  description:
    "Yelkenli yat: rüzgardan daha fazla etkilenir, daha ağır döner, marina manevrasında eğitim hissi güçlüdür.",
  length: 92,
  width: 30,
  drag: 0.985,
  rudderPower: 0.82,
...
const SCENARIOS: Record<ScenarioKey, ScenarioConfig> = {
  training: {
    ...
    windForce: 0.35,
    difficulty: "NORMAL",
    start: { x: 240, y: 520, heading: 90 },
    target: { x: 610, y: 500, heading: 0 },
  },
  night: {
...
function getBoatGeometry(x: number, y: number, heading: number, boatConfig: BoatConfig) {
  const rad = (heading * Math.PI) / 180;
  const sideRad = ((heading + 90) * Math.PI) / 180;

  return {
    bowX: x + Math.sin(rad) * boatConfig.length * 0.5,
    bowY: y - Math.cos(rad) * boatConfig.length * 0.5,
    sternX: x - Math.sin(rad) * boatConfig.length * 0.5,
    sternY: y + Math.cos(rad) * boatConfig.length * 0.5,
    portX: x - Math.sin(sideRad) * boatConfig.width * 0.5,
    portY: y + Math.cos(sideRad) * boatConfig.width * 0.5,
    starboardX: x + Math.sin(sideRad) * boatConfig.width * 0.5,
    starboardY: y - Math.cos(sideRad) * boatConfig.width * 0.5,
  };
}

function boatHitsObstacle(x: number, y: number, heading: number, boatConfig: BoatConfig) {
  ...
  return OBSTACLES.some((obstacle) =>
    points.some((point) => pointInRect(point.x, point.y, obstacle))
  );
}
...
export default function MarinaDockingSimulator() {
  ...
  const activeTarget = SCENARIOS[scenario].target;
  ...
  const targetGeometry = useMemo(
    () => getBoatGeometry(activeTarget.x, activeTarget.y, activeTarget.heading, activeBoat),
    [activeTarget, activeBoat]
  );
  ...
  const bowDistance = useMemo(
    () => distance(boatGeometry.bowX, boatGeometry.bowY, targetGeometry.bowX, targetGeometry.bowY),
  ...
  const dockingStatus = useMemo(() => {
    ...
    if (bowDistance < 52 && sternDistance < 52 && Math.abs(speed) < 0.85 && dockingAngleError < 22) {
      return "READY TO SECURE";
    }

    if (dockingDistance < 100 && Math.abs(speed) < 1.05) return "FINAL APPROACH";
  ...
  const maneuverAdvice = useMemo(() => {
    ...
    if (result === "FAILED") return "Hedef alana hızlı girdin. Son yaklaşmada gazı kes ve rüzgarla süzül.";
    if (collisionRisk && showObstacles) return "Çarpışma bölgesine çok yakınsın. Hızı düşür ve baş açını güvenli suya çevir.";
    if (Math.abs(speed) > 2.1) return "Hız yüksek. Yanaşma öncesi gazı azalt, tekneyi süzülmeye bırak.";
    if (dockingAngleError > 35 && dockingDistance < 150) return "Açı hatası büyük. Baş-kıç hattını hedef iskele çizgisine paralel hale getir.";
    if (bowDistance > sternDistance + 35) return "Baş taraf geride kalıyor. Bow thruster veya kısa dümenle başı hedefe yaklaştır.";
    if (sternDistance > bowDistance + 35) return "Kıç taraf geride kalıyor. İleri/geri küçük hareketlerle kıçı hizala.";
    if (windForce > 0.7) return "Rüzgar etkisi belirgin. Rüzgaraltına düşmeden önce küçük düzeltmeler yap.";
    if (dockingDistance < 100) return "Son yaklaşma, çok küçük gaz, küçük dümen ve düşük hızla ilerle.";
  ...
  function handleMobilePointer(e: React.PointerEvent<HTMLDivElement>) {
    ...
    const dx = clamp(e.clientX - centerX, -54, 54);
    const dy = clamp(e.clientY - centerY, -54, 54);

    const rudderValue = clamp(dx / 54, -1, 1);
    const throttleValue = clamp(-dy / 54, -1, 1);

    controlsRef.current.rudder = rudderValue;
    controlsRef.current.throttle =
      throttleValue < 0 ? throttleValue * activeBoat.reversePower : throttleValue;

    setRudder(rudderValue);
    setMobileStick({ x: dx, y: dy });
    setMobileActive(true);
  }

  function resetSimulatorForScenario(targetScenario: ScenarioKey, incrementAttempt = true) {
    ...
    if (incrementAttempt) setAttempts((value) => value + 1);
  }
  ...
  useEffect(() => {
    const down = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowDown") controlsRef.current.throttle = -activeBoat.reversePower;
    ...
    const up = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowUp" || e.key === "ArrowDown") controlsRef.current.throttle = 0;
      if (e.key === "ArrowLeft" || e.key === "ArrowRight") controlsRef.current.rudder = 0;
      if (e.key.toLowerCase() === "a" || e.key.toLowerCase() === "d") controlsRef.current.bowThruster = 0;
    };
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      if (resultRef.current === "RUNNING") {
        const scenarioDrag =
          scenario === "tight" ? activeBoat.drag - 0.003 : scenario === "windy" ? activeBoat.drag - 0.004 : activeBoat.drag;
        ...
        boat.heading += controls.rudder * boat.speed * activeBoat.rudderPower * rudderFactor;
        boat.heading += controls.bowThruster * activeBoat.bowThrusterPower * clamp(1 - Math.abs(boat.speed) * 0.35, 0.25, 1);

        const windSideArea = Math.abs(Math.sin(((windDirection - boat.heading) * Math.PI) / 180));
        const windDriftPower = windForce * activeBoat.windDriftFactor * (0.11 + windSideArea * 0.12);
        const windYaw =
        ...
        if (frameCountRef.current % 5 === 0 && Math.abs(boat.speed) > 0.08) {
          ...
          const sternX = boat.x - Math.sin(wakeRad) * activeBoat.length * 0.5;
          const sternY = boat.y + Math.cos(wakeRad) * activeBoat.length * 0.5;
          const intensity = Math.min(1, Math.abs(boat.speed) / 3.2) * activeBoat.wakeFactor;
          ...
          setWakeTrail((previous) => [
            ...
            ...previous
              .map((point) => ({ ...point, life: point.life - 0.07 }))
              .filter((point) => point.life > 0)
              .slice(0, 34),
          ]);
        } else if (frameCountRef.current % 5 === 0) {
          setWakeTrail((previous) =>
            previous.map((point) => ({ ...point, life: point.life - 0.07 })).filter((point) => point.life > 0)
          );
        }

        const hitObstacle = boatHitsObstacle(boat.x, boat.y, boat.heading, activeBoat);
        const liveGeometry = getBoatGeometry(boat.x, boat.y, boat.heading, activeBoat);
        ...
        if (showObstacles && hitObstacle && Math.abs(boat.speed) > 1.6) {
        ...
        const tooFastInBerth =
          liveBowDistance < 38 && liveSternDistance < 38 && Math.abs(boat.speed) >= 0.9;
  ...
  }, [windDirection, windForce, scenario, targetGeometry, activeTarget.heading, activeBoat, showObstacles]);
    ...
    <main className="relative min-h-screen overflow-hidden bg-[#07111f] text-white">
      ...
      <section className="relative border-b border-cyan-400/10">
        <div className="mx-auto max-w-7xl px-6 pb-14 pt-20">
          ...
          <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-300">
            Simulator Marina ve Sailing Yacht ile gerçekçi iskele yanaşma eğitimi.
          </p>
        </div>
      </section>
      ...
      <section className="mx-auto grid max-w-7xl gap-6 px-6 py-10 lg:grid-cols-[340px_1fr]">
        <aside className="rounded-[32px] border border-cyan-400/15 bg-white/5 p-6 backdrop-blur-xl">
          <h2 className="text-2xl font-black text-cyan-300">Bridge Controls</h2>

          <div className="mt-6 rounded-3xl border border-white/10 bg-white/5 p-5">
            <p className="text-xs font-black uppercase tracking-[0.22em] text-slate-300">
              Marina / Sahne
            </p>
            <p className="mt-3 rounded-2xl border border-cyan-300/20 bg-cyan-300/10 px-4 py-3 text-sm font-black text-cyan-100">
              {MARINA.label}
            </p>
            <p className="mt-3 text-xs leading-5 text-slate-400">{MARINA.description}</p>
          </div>
          ...
          <div className="mt-6 rounded-3xl border border-white/10 bg-white/5 p-5">
            ...
            <p className="mt-3 text-xs leading-5 text-slate-400">{BOAT.description}</p>
          </div>
          ...
          <div className="mt-6 rounded-3xl border border-white/10 bg-white/5 p-5">
            ...
            <p className="mt-3 text-xs leading-5 text-slate-400">{activeScenario.description}</p>
          ...
          <div className="mt-6 space-y-4">
            <ControlCard title="Mobil Joystick" text="Sağ alttaki daireyi sürükle: gaz + dümen." />
            <ControlCard title="↑ Forward Throttle" text="Klavye: ileri gaz" />
            <ControlCard title="↓ Reverse" text="Klavye: geri manevra" />
            <ControlCard title="← → Rudder" text="Klavye: dümen" />
            <ControlCard title="A / D Bow Thruster" text="Klavye: baş itici" />
          </div>
          ...
          <div className="mt-6 grid grid-cols-2 gap-3">
            <ToggleButton label={`NIGHT ${nightMode ? "ON" : "OFF"}`} onClick={() => setNightMode((value) => !value)} />
            <ToggleButton label={`AIS ${aisOverlay ? "ON" : "OFF"}`} onClick={() => setAisOverlay((value) => !value)} />
            <ToggleButton label={`RADAR ${radarOverlay ? "ON" : "OFF"}`} onClick={() => setRadarOverlay((value) => !value)} />
            <ToggleButton
              label={`CAM ${cameraMode === "cinematic" ? "CINE" : "NORMAL"}`}
              onClick={() => setCameraMode((value) => (value === "normal" ? "cinematic" : "normal"))}
            />
            <ToggleButton label={`OBSTACLE ${showObstacles ? "ON" : "OFF"}`} onClick={() => setShowObstacles((value) => !value)} />
          </div>

          <div className="mt-6 rounded-3xl border border-cyan-400/20 bg-cyan-400/10 p-5">
            <StatusRow label="Heading" value={`${Math.round(normalizeHeading(heading))}°`} />
            <StatusRow label="Speed" value={speed.toFixed(2)} />
            <StatusRow label="Rudder" value={rudder < -0.1 ? "PORT" : rudder > 0.1 ? "STARBOARD" : "MID"} />
            <StatusRow label="Bow Thruster" value={bowThruster === -1 ? "PORT" : bowThruster === 1 ? "STARBOARD" : "OFF"} />
            <StatusRow label="Position" value={`${Math.round(x)} / ${Math.round(y)}`} />
            <StatusRow label="Attempt" value={String(attempts)} />
            <StatusRow label="Bow Distance" value={`${Math.round(bowDistance)} px`} />
            <StatusRow label="Stern Distance" value={`${Math.round(sternDistance)} px`} />
            <StatusRow label="Angle Error" value={`${Math.round(dockingAngleError)}°`} />
        ...
        <section
          className="relative overflow-hidden rounded-[32px] border border-cyan-400/15 bg-white/5"
        ...
        >
          ...
          <div className="relative z-10 h-[780px] w-full overflow-hidden">
            ...
            >
              <img
                src={MARINA.background}
                alt={MARINA.label}
                className="absolute inset-0 h-full w-full object-cover"
              />

              <div className={`absolute inset-0 ${nightMode ? "bg-black/82" : "bg-slate-950/20"}`} />
              ...
              {wakeTrail.map((point) => (
                <div
                  ...
                  style={{
                    ...
                    transform: `translate(-50%, -50%) rotate(${point.heading}deg) scale(${0.6 + (1 - point.life) * 1.6})`,
                  }}
                >
                  <div className="h-28 w-10 rounded-full bg-cyan-100/25 blur-xl" style={{ opacity: point.intensity }} />
                  <div className="absolute left-1/2 top-2 h-16 w-20 -translate-x-1/2 rounded-full border border-white/15" style={{ opacity: point.intensity * 0.75 }} />
                </div>
              ))}
              ...
              >
                <div
                  ...
                  style={{ height: activeBoat.length + 18, width: activeBoat.width + 42 }}
                />
              ...
              {mooringLines ? (
                <>
                  <div className="absolute z-[24] h-[3px] origin-left rounded-full bg-amber-200 shadow-[0_0_18px_rgba(251,191,36,0.65)]" style={{ left: x, top: y - 24, width: 100, transform: "rotate(18deg)" }} />
                  <div className="absolute z-[24] h-[3px] origin-left rounded-full bg-amber-200 shadow-[0_0_18px_rgba(251,191,36,0.65)]" style={{ left: x, top: y + 22, width: 112, transform: "rotate(-22deg)" }} />
                </>
              ) : null}
              ...
              >
                <div
                  ...
                  style={{ opacity: Math.min(0.75, Math.abs(speed) / 3) }}
                />
                ...
                <img
                  ...
                  style={{ width: activeBoat.length + 45 }}
                  draggable={false}
                />
              </div>
            </div>
            ...
            <div className="absolute bottom-5 left-5 z-30 max-w-sm rounded-2xl border border-cyan-300/25 bg-slate-950/75 p-4 text-xs backdrop-blur">
              <p className="font-black uppercase tracking-[0.25em] text-cyan-200">Manevra Tavsiyesi</p>
            ...
            {aisOverlay ? (
              <div className="absolute left-5 top-5 z-30 w-56 rounded-2xl border border-cyan-300/25 bg-slate-950/75 p-4 text-xs backdrop-blur">
                <p className="font-black uppercase tracking-[0.25em] text-cyan-200">AIS Overlay</p>
                <div className="mt-3 space-y-2 text-slate-300">
                  <AISRow label="Vessel" value={activeBoat.label} />
                  <AISRow label="COG" value={`${Math.round(normalizeHeading(heading))}°`} />
                  <AISRow label="SOG" value={Math.abs(speed).toFixed(2)} />
                  <AISRow label="Bow" value={`${Math.round(bowDistance)} px`} />
                  <AISRow label="Stern" value={`${Math.round(sternDistance)} px`} />
            ...
            {radarOverlay ? (
              <div className="absolute bottom-5 right-5 z-[80] flex flex-col items-center gap-3">
                <div
                  className="relative h-44 w-44 rounded-full border border-emerald-300/35 bg-slate-950/80 p-4 shadow-[0_0_35px_rgba(34,211,238,0.18)] backdrop-blur-xl"
                  style={{
                    touchAction: "none",
                  ...
                  onPointerDown={(e) => {
                    ...
                    handleMobilePointer(e);
                  }}
                  onPointerMove={(e) => {
                    if (mobileActive) handleMobilePointer(e);
                  }}
                  onPointerUp={(e) => {
                    e.currentTarget.releasePointerCapture(e.pointerId);
                    stopMobileControl();
                  }}
                  onPointerCancel={stopMobileControl}
                >
                  <div className="relative h-full w-full rounded-full border border-emerald-300/20">
                    ...
                    <div
                      className="absolute left-1/2 top-1/2 h-11 w-11 rounded-full bg-cyan-300 shadow-[0_0_25px_rgba(103,232,249,0.95)]"
                      style={{
                        transform: `translate(calc(-50% + ${mobileStick.x}px), calc(-50% + ${mobileStick.y}px))`,
                      }}
                    />

                    <div className="pointer-events-none absolute bottom-2 left-1/2 -translate-x-1/2 text-[9px] font-black uppercase tracking-[0.18em] text-cyan-100/80">
                      Touch Control
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    ...
                    onPointerDown={() => {
                      controlsRef.current.bowThruster = -1;
                      setBowThruster(-1);
                    }}
                    onPointerUp={() => {
            ...
            {collisionFlash ? <div className="pointer-events-none absolute inset-0 z-[38] animate-pulse bg-red-500/20" /> : null}
            ...
            {result !== "RUNNING" ? (
              <div className="absolute inset-0 z-40 flex items-center justify-center bg-slate-950/45 backdrop-blur-sm">
                ...
                >
                  <p className="text-xs font-black uppercase tracking-[0.3em] text-white/70">Training Result</p>
